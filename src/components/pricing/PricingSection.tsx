...
export default function PricingSection() {
    ...
    `}>
                            ...
                            <h2 className="text-2xl font-semibold mb-2"> {p.plan_name}</h2>
                            <p className="text-lg text-gray-300 mb-6">{p.about}</p>
                            <p className="text-4xl font-bold mb-4">{p.price}<span className="text-lg text-gray-400">/month</span></p>

                            {
                                p.plan_name.toLocaleLowerCase() === plan && isActive ? (
                                    <h1 className="w-full text-center py-2 rounded-xl border border-gray-400  bg-green-500 hover:text-black transition hover:cursor-pointer">Active Plan</h1>
                                ) : (
                                    <button onClick={() => handleSubscriptionPlan(p.plan_name.toLocaleLowerCase())} className="w-full py-2 rounded-xl border border-gray-400 hover:bg-white hover:text-black transition hover:cursor-pointer">{p.button}</button>
